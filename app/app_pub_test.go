...
func setupAppTest(t *testing.T) (*assert.Assertions, *require.Assertions, *Aximchain, sdk.Account, sdk.Account) {
	logger := log.NewNopLogger()
	db := dbm.NewMemDB()

	app := NewAximchain(logger, db, os.Stdout)
	...
	appState, _ := AximchainAppGenState(app.Codec, []json.RawMessage{genTx})
	...
	tradingPair := dextypes.NewTradingPair("XYZ-000", "AXC", 102000)
	keeper.PairMapper.AddTradingPair(app.DeliverState.Ctx, tradingPair)
	keeper.AddEngine(tradingPair)
	tradingPair = dextypes.NewTradingPair("ZCB-000", "AXC", 102000)
	keeper.PairMapper.AddTradingPair(app.DeliverState.Ctx, tradingPair)
	keeper.AddEngine(tradingPair)
...
func TestAppPub_AddOrder(t *testing.T) {
	...
	msg := orderPkg.NewNewOrderMsg(buyerAcc.GetAddress(), "1", orderPkg.Side.BUY, "XYZ-000_AXC", 102000, 3000000)
	...
	assert.Equal(pub.OrderBookDelta{"XYZ-000_AXC", []pub.PriceLevel{{102000, 3000000}}, make([]pub.PriceLevel, 0)}, publisher.BooksPublished[0].Books[0])
...
func TestAppPub_MatchOrder(t *testing.T) {
	...
	msg := orderPkg.NewNewOrderMsg(buyerAcc.GetAddress(), orderPkg.GenerateOrderID(1, buyerAcc.GetAddress()), orderPkg.Side.BUY, "XYZ-000_AXC", 102000, 300000000)
	...
	expectedAccountToPub := pub.Account{string(buyerAcc.GetAddress()), "", 1, []*pub.AssetBalance{{"AXC", 99999694000, 0, 306000}, {"XYZ-000", 100000000000, 0, 0}}}
	...
	msg = orderPkg.NewNewOrderMsg(sellerAcc.GetAddress(), orderPkg.GenerateOrderID(1, sellerAcc.GetAddress()), orderPkg.Side.SELL, "XYZ-000_AXC", 102000, 400000000)
	...
	assert.Equal(pub.OrderBookDelta{"XYZ-000_AXC", []pub.PriceLevel{{102000, 0}}, []pub.PriceLevel{{102000, 100000000}}}, publisher.BooksPublished[1].Books[0])
	expectedAccountToPub = pub.Account{string(buyerAcc.GetAddress()), "AXC:153", 1, []*pub.AssetBalance{{"AXC", 99999693847, 0, 0}, {"XYZ-000", 100300000000, 0, 0}}}
	expectedAccountToPubSeller := pub.Account{string(sellerAcc.GetAddress()), "AXC:153", 1, []*pub.AssetBalance{{"AXC", 100000305847, 0, 0}, {"XYZ-000", 99600000000, 0, 100000000}}}
	...
	msg = orderPkg.NewNewOrderMsg(buyerAcc.GetAddress(), orderPkg.GenerateOrderID(2, buyerAcc.GetAddress()), orderPkg.Side.BUY, "XYZ-000_AXC", 102000, 100000000)
	ctx = ctx.WithBlockHeight(43).WithBlockTime(time.Unix(0, 102))
	buyerAcc.SetSequence(2)
	app.AccountKeeper.SetAccount(ctx, buyerAcc)
	res = handler(ctx, msg)
	msg = orderPkg.NewNewOrderMsg(sellerAcc.GetAddress(), orderPkg.GenerateOrderID(2, sellerAcc.GetAddress()), orderPkg.Side.SELL, "XYZ-000_AXC", 102000, 100000000)
	...
	expectedAccountToPub = pub.Account{string(buyerAcc.GetAddress()), "AXC:51", 2, []*pub.AssetBalance{{"AXC", 99999897949, 0, 0}, {"XYZ-000", 100100000000, 0, 0}}}
	expectedAccountToPubSeller = pub.Account{string(sellerAcc.GetAddress()), "AXC:51", 2, []*pub.AssetBalance{{"AXC", 100000101949, 0, 0}, {"XYZ-000", 99900000000, 0, 0}}}
...
func TestAppPub_MatchAndCancelFee(t *testing.T) {
	...
	msg := orderPkg.NewNewOrderMsg(sellerAcc.GetAddress(), orderPkg.GenerateOrderID(1, sellerAcc.GetAddress()), orderPkg.Side.SELL, "XYZ-000_AXC", 102000, 100000000)
	...
	msg2 := orderPkg.NewNewOrderMsg(buyerAcc.GetAddress(), orderPkg.GenerateOrderID(1, buyerAcc.GetAddress()), orderPkg.Side.BUY, "ZCB-000_AXC", 102000, 100000000)
	buyerAcc.SetSequence(1)
	app.AccountKeeper.SetAccount(ctx, buyerAcc)
	...
	msg3 := orderPkg.NewNewOrderMsg(buyerAcc.GetAddress(), orderPkg.GenerateOrderID(2, buyerAcc.GetAddress()), orderPkg.Side.BUY, "XYZ-000_AXC", 102000, 100000000)
	ctx = ctx.WithBlockHeight(42).WithBlockTime(time.Unix(0, 101))
	...
	cxlMsg := orderPkg.NewCancelOrderMsg(buyerAcc.GetAddress(), "ZCB-000_AXC", orderPkg.GenerateOrderID(1, buyerAcc.GetAddress()))
	buyerAcc = app.AccountKeeper.GetAccount(ctx, buyerAcc.GetAddress())
	...
	assert.Equal("AXC:51", publisher.ExecutionResultsPublished[1].Trades.Trades[0].Sfee)
	assert.Equal("AXC:57;#Cxl:1", publisher.ExecutionResultsPublished[1].Trades.Trades[0].Bfee)
	assert.Equal("AXC:108", publisher.BlockFeePublished[1].Fee)
	publisher.Lock.Unlock()
}
