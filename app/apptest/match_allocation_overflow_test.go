...
func Test_Overflow_1a(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		msg := order.NewNewOrderMsg(addr0, oid, 1, "BTC-000_AXC", 1, 1e18)
...
func Test_Overflow_1b(t *testing.T) {
	...
	*/
	...
	for i := 0; i < 10; i++ {
		oid := GetOrderId(addr0, int64(i), ctx)
		msg := order.NewNewOrderMsg(addr0, oid, 1, "BTC-000_AXC", int64(i+1), 1e18)
		res, err := testClient.DeliverTxSync(msg, testApp.Codec)
		assert.NoError(err)
		assert.Equal(uint32(0), res.Code)
	}
	...
	msgS1 := order.NewNewOrderMsg(addr1, oidS, 2, "BTC-000_AXC", int64(1), 1e13)
	...
	assert.Equal(int64(94500e8), GetAvail(ctx, addr0, "AXC"))
	assert.Equal(int64(5500e8), GetLocked(ctx, addr0, "AXC"))
	...
	assert.Equal(int64(94499.99999500e8), GetAvail(ctx, addr0, "AXC"))
	assert.Equal(int64(5499.9900e8), GetLocked(ctx, addr0, "AXC"))
	assert.Equal(int64(0), GetAvail(ctx, addr1, "BTC-000"))
	assert.Equal(int64(100000.00999500e8), GetAvail(ctx, addr1, "AXC"))
...
func Test_Overflow_1c(t *testing.T) {
	...
	*/
	...
	msgS1 := order.NewNewOrderMsg(addr1, oidS1, 2, "BTC-000_AXC", int64(1), 1e9)
	res, err := testClient.DeliverTxSync(msgS1, testApp.Codec)
	assert.NoError(err)
	assert.Equal(uint32(0), res.Code)
	...
	// fee charged from receiving token btc-000, as fee in axc is < 1
	assert.Equal(int64(100009.9900e8), GetAvail(ctx, addr0, "BTC-000"))
	assert.Equal(int64(94500e8), GetAvail(ctx, addr0, "AXC"))
	assert.Equal(int64(5499.99999900e8), GetLocked(ctx, addr0, "AXC"))
	// in this case, it is expected that no fee charged for sell side
	assert.Equal(int64(99990e8), GetAvail(ctx, addr1, "BTC-000"))
	assert.Equal(int64(100000.00000100e8), GetAvail(ctx, addr1, "AXC"))
	assert.Equal(int64(0), GetLocked(ctx, addr1, "BTC-000"))
}

/*
test #1d: additional test case using very cheap axc, not really overflow related
*/
func Test_Overflow_1d(t *testing.T) {
	...
	msgB1 := order.NewNewOrderMsg(addr0, oidB1, 1, "BTC-000_AXC", 1e18, 1)
	_, err := testClient.DeliverTxSync(msgB1, testApp.Codec)
	assert.NoError(err)

	oidS1 := GetOrderId(addr1, 0, ctx)
	msgS1 := order.NewNewOrderMsg(addr1, oidS1, 2, "BTC-000_AXC", 1e18, 1)
	...
	assert.Equal(int64(1999999989995000000), GetAvail(ctx, addr0, "AXC"))
	assert.Equal(int64(0), GetLocked(ctx, addr0, "AXC"))
	assert.Equal(int64(9999999999999), GetAvail(ctx, addr1, "BTC-000"))
	assert.Equal(int64(2000000009995000000), GetAvail(ctx, addr1, "AXC"))
	assert.Equal(int64(0), GetLocked(ctx, addr1, "BTC-000"))
}
...
func Test_Overflow_2a(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		oid := GetOrderId(addr0, int64(i), ctx)
		msg := order.NewNewOrderMsg(addr0, oid, 2, "BTC-000_AXC", 1e18, 1e8)
		res, err := testClient.DeliverTxSync(msg, testApp.Codec)
		assert.NoError(err)
		assert.Equal(uint32(0), res.Code)
	}
...
func Test_Overflow_2b(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		oid := GetOrderId(addr0, int64(i), ctx)
		msg := order.NewNewOrderMsg(addr0, oid, 2, "BTC-000_AXC", 1e18*int64(i+1), 1e8)
		res, err := testClient.DeliverTxSync(msg, testApp.Codec)
		assert.NoError(err)
		assert.Equal(uint32(0), res.Code)
	}
...
/*
test #3: non axc pair (with cheap axc)
*/
func Test_Overflow_3(t *testing.T) {
	...
	// for buy side: insufficient axc (1x1e18 > 100000e8), so fee is deducted from btc-000 => 100001 - 1 * 0.001
...
/*
test #4: non axc pair (with expansive axc)
*/
...
func Test_Overflow_5(t *testing.T) {
	...
	*/
	...
	oidB1 := GetOrderId(addr0, 0, ctx)
	msgB1 := order.NewNewOrderMsg(addr0, oidB1, 1, "BTC-000_AXC", 5, 5e18)
	_, err := testClient.DeliverTxSync(msgB1, testApp.Codec)
	assert.NoError(err)

	oidB2 := GetOrderId(addr0, 1, ctx)
	msgB2 := order.NewNewOrderMsg(addr0, oidB2, 1, "BTC-000_AXC", 4, 5e18)
	_, err = testClient.DeliverTxSync(msgB2, testApp.Codec)
	assert.NoError(err)

	oidS1 := GetOrderId(addr1, 0, ctx)
	msgS1 := order.NewNewOrderMsg(addr1, oidS1, 2, "BTC-000_AXC", 3, 9e18)
	_, err = testClient.DeliverTxSync(msgS1, testApp.Codec)
	assert.NoError(err)
	...
	assert.Equal(int64(96898.6500e8), GetAvail(ctx, addr0, "AXC"))
	assert.Equal(int64(400e8), GetLocked(ctx, addr0, "AXC"))
	assert.Equal(int64(0), GetAvail(ctx, addr1, "BTC-000"))
	assert.Equal(int64(102698.6500e8), GetAvail(ctx, addr1, "AXC"))
	assert.Equal(int64(0), GetLocked(ctx, addr1, "BTC-000"))
}
