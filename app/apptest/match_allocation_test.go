...
import (
	...
	param "github.com/aximchain/axc-cosmos-sdk/x/paramHub"
...
func SetupTest(initPrices ...int64) (crypto.Address, sdk.Context, []sdk.Account) {
	...
	genTokens := []tokens.GenesisToken{{"AXC", "AXC", 100000000e8, accAddr, false}}
	...
	axcbtcPair := types.NewTradingPair("BTC-000", "AXC", prices[0])
	testApp.DexKeeper.PairMapper.AddTradingPair(ctx, axcbtcPair)
	testApp.DexKeeper.AddEngine(axcbtcPair)
	axcethPair := types.NewTradingPair("ETH-000", "AXC", prices[1])
	testApp.DexKeeper.PairMapper.AddTradingPair(ctx, axcethPair)
	testApp.DexKeeper.AddEngine(axcethPair)
	...
	testApp.DexKeeper.ClearOrderBook("BTC-000_AXC")
	testApp.DexKeeper.ClearOrderBook("ETH-000_AXC")
	testApp.DexKeeper.ClearOrderBook("BTC-000_ETH-000")
	testApp.DexKeeper.FeeManager.UpdateConfig(testFeeConfig)
	coins := sdk.Coins{
		sdk.NewCoin("AXC", 100000e8),
...
func Test_Match_Allocation(t *testing.T) {
	...
	msg1_1 := order.NewNewOrderMsg(addr0, GetOrderId(addr0, 0, ctx), 1, "BTC-000_AXC", 1e8, 75000e8)
	_, err := testClient.DeliverTxSync(msg1_1, testApp.Codec)
	assert.NoError(err)

	msg1_2 := order.NewNewOrderMsg(addr0, GetOrderId(addr0, 1, ctx), 1, "BTC-000_AXC", 1e8, 75000e8)
	...
	assert.Equal(int64(25000e8), GetAvail(ctx, addr0, "AXC"))
	assert.Equal(int64(75000e8), GetLocked(ctx, addr0, "AXC"))

	//#2 cannot sell more than they have
	msg2_1 := order.NewNewOrderMsg(addr1, GetOrderId(addr1, 0, ctx), 2, "BTC-000_AXC", 1e8, 60000e8)
	_, err = testClient.DeliverTxSync(msg2_1, testApp.Codec)
	assert.NoError(err)

	msg2_2 := order.NewNewOrderMsg(addr1, GetOrderId(addr1, 1, ctx), 2, "BTC-000_AXC", 1e8, 60000e8)
	...
	msg3_1 := order.NewCancelOrderMsg(addr0, "BTC-000_AXC", msg1_1.Id)
	...
	msg3_2 := order.NewNewOrderMsg(addr0, GetOrderId(addr0, 2, ctx), 1, "BTC-000_AXC", 1e8, 75000e8)
	_, err = testClient.DeliverTxSync(msg3_2, testApp.Codec)
	assert.NoError(err)

	assert.Equal(int64(100000e8), GetAvail(ctx, addr0, "BTC-000"))
	assert.Equal(int64(24999.9998e8), GetAvail(ctx, addr0, "AXC"))
	assert.Equal(int64(75000e8), GetLocked(ctx, addr0, "AXC"))

	msg3_3 := order.NewCancelOrderMsg(addr0, "BTC-000_AXC", msg3_2.Id)
	_, err = testClient.DeliverTxSync(msg3_3, testApp.Codec)
	assert.NoError(err)

	assert.Equal(int64(100000e8), GetAvail(ctx, addr0, "BTC-000"))
	assert.Equal(int64(99999.9996e8), GetAvail(ctx, addr0, "AXC"))

	msg3_4 := order.NewCancelOrderMsg(addr1, "BTC-000_AXC", msg2_1.Id)
	...
	*/
	...
	msg := order.NewNewOrderMsg(addr2, GetOrderId(addr2, 0, ctx), 1, "BTC-000_AXC", 12e8, 30e8)
	res, err = testClient.DeliverTxSync(msg, testApp.Codec)

	msg = order.NewNewOrderMsg(addr2, GetOrderId(addr2, 1, ctx), 1, "BTC-000_AXC", 10e8, 10e8)
	res, err = testClient.DeliverTxSync(msg, testApp.Codec)

	msg = order.NewNewOrderMsg(addr2, GetOrderId(addr2, 2, ctx), 1, "BTC-000_AXC", 9e8, 20e8)
	res, err = testClient.DeliverTxSync(msg, testApp.Codec)

	msg = order.NewNewOrderMsg(addr2, GetOrderId(addr2, 3, ctx), 1, "BTC-000_AXC", 8e8, 30e8)
	res, err = testClient.DeliverTxSync(msg, testApp.Codec)

	msg = order.NewNewOrderMsg(addr3, GetOrderId(addr3, 0, ctx), 2, "BTC-000_AXC", 8e8, 25e8)
	res, err = testClient.DeliverTxSync(msg, testApp.Codec)

	msg = order.NewNewOrderMsg(addr3, GetOrderId(addr3, 1, ctx), 2, "BTC-000_AXC", 7e8, 25e8)
	res, err = testClient.DeliverTxSync(msg, testApp.Codec)

	msg = order.NewNewOrderMsg(addr3, GetOrderId(addr3, 2, ctx), 2, "BTC-000_AXC", 6e8, 100e8)
	res, err = testClient.DeliverTxSync(msg, testApp.Codec)
	...
	assert.Equal(int64(99459.73e8), GetAvail(ctx, addr2, "AXC"))
	...
	assert.Equal(int64(100539.73e8), GetAvail(ctx, addr3, "AXC"))
	...
	/*
	   ...
	   20     20      6               15     15      -5
	*/
	msg = order.NewNewOrderMsg(addr2, GetOrderId(addr2, 4, ctx), 1, "BTC-000_AXC", 12e8, 15e8)
	res, err = testClient.DeliverTxSync(msg, testApp.Codec)

	msg = order.NewNewOrderMsg(addr1, GetOrderId(addr1, 1, ctx), 2, "BTC-000_AXC", 6e8, 10e8)
	res, err = testClient.DeliverTxSync(msg, testApp.Codec)
	...
	assert.Equal(int64(99369.6850e8), GetAvail(ctx, addr2, "AXC"))
	assert.Equal(int64(0), GetLocked(ctx, addr2, "BTC-000"))
	assert.Equal(int64(0), GetLocked(ctx, addr2, "AXC"))
	assert.Equal(int64(99850e8), GetAvail(ctx, addr3, "BTC-000"))
	assert.Equal(int64(100599.7e8), GetAvail(ctx, addr3, "AXC"))
	...
	*/
	...
	msg = order.NewNewOrderMsg(addr2, GetOrderId(addr2, 5, ctx), 1, "BTC-000_AXC", 12e8, 5e8)
	res, err = testClient.DeliverTxSync(msg, testApp.Codec)

	msg = order.NewNewOrderMsg(addr2, GetOrderId(addr2, 6, ctx), 1, "BTC-000_AXC", 10e8, 5e8)
	res, err = testClient.DeliverTxSync(msg, testApp.Codec)

	msg = order.NewNewOrderMsg(addr2, GetOrderId(addr2, 7, ctx), 1, "BTC-000_AXC", 8e8, 10e8)
	res, err = testClient.DeliverTxSync(msg, testApp.Codec)

	msg = order.NewNewOrderMsg(addr2, GetOrderId(addr2, 8, ctx), 1, "BTC-000_AXC", 7e8, 5e8)
	res, err = testClient.DeliverTxSync(msg, testApp.Codec)
	...
	assert.Equal(int64(99194.5975e8), GetAvail(ctx, addr2, "AXC"))
	assert.Equal(int64(0), GetLocked(ctx, addr2, "BTC-000"))
	assert.Equal(int64(0), GetLocked(ctx, addr2, "AXC"))
	assert.Equal(int64(99850e8), GetAvail(ctx, addr3, "BTC-000"))
	assert.Equal(int64(100739.63e8), GetAvail(ctx, addr3, "AXC"))
	assert.Equal(int64(30e8), GetLocked(ctx, addr3, "BTC-000"))
	assert.Equal(int64(0), GetLocked(ctx, addr3, "AXC"))
	assert.Equal(int64(99990e8), GetAvail(ctx, addr1, "BTC-000"))
	assert.Equal(int64(100064.9673e8), GetAvail(ctx, addr1, "AXC"))
	...
	*/
	...
	msg = order.NewNewOrderMsg(addr2, GetOrderId(addr2, 9, ctx), 1, "BTC-000_AXC", 13e8, 10e8)
	res, err = testClient.DeliverTxSync(msg, testApp.Codec)

	msg = order.NewNewOrderMsg(addr2, GetOrderId(addr2, 10, ctx), 1, "BTC-000_AXC", 10e8, 10e8)
	res, err = testClient.DeliverTxSync(msg, testApp.Codec)

	msg = order.NewNewOrderMsg(addr3, GetOrderId(addr3, 3, ctx), 2, "BTC-000_AXC", 8e8, 25e8)
	res, err = testClient.DeliverTxSync(msg, testApp.Codec)
	...
	assert.Equal(int64(99034.5175e8), GetAvail(ctx, addr2, "AXC"))
	assert.Equal(int64(0), GetLocked(ctx, addr2, "BTC-000"))
	assert.Equal(int64(0), GetLocked(ctx, addr2, "AXC"))
	assert.Equal(int64(99825e8), GetAvail(ctx, addr3, "BTC-000"))
	assert.Equal(int64(100899.55e8), GetAvail(ctx, addr3, "AXC"))
	...
	*/
	...
	msg = order.NewNewOrderMsg(addr2, GetOrderId(addr2, 11, ctx), 1, "BTC-000_AXC", 13e8, 10e8)
	res, err = testClient.DeliverTxSync(msg, testApp.Codec)

	msg = order.NewNewOrderMsg(addr3, GetOrderId(addr3, 4, ctx), 2, "BTC-000_AXC", 11e8, 10e8)
	res, err = testClient.DeliverTxSync(msg, testApp.Codec)

	msg = order.NewNewOrderMsg(addr2, GetOrderId(addr2, 12, ctx), 1, "BTC-000_AXC", 10e8, 10e8)
	res, err = testClient.DeliverTxSync(msg, testApp.Codec)
	...
	assert.Equal(int64(98874.4375e8), GetAvail(ctx, addr2, "AXC"))
	assert.Equal(int64(0), GetLocked(ctx, addr2, "BTC-000"))
	assert.Equal(int64(0), GetLocked(ctx, addr2, "AXC"))
	assert.Equal(int64(99815e8), GetAvail(ctx, addr3, "BTC-000"))
	assert.Equal(int64(101059.47e8), GetAvail(ctx, addr3, "AXC"))
	...
	*/
	...
	msg = order.NewNewOrderMsg(addr2, GetOrderId(addr2, 13, ctx), 1, "BTC-000_AXC", 12e8, 10e8)
	res, err = testClient.DeliverTxSync(msg, testApp.Codec)

	msg = order.NewNewOrderMsg(addr2, GetOrderId(addr2, 14, ctx), 1, "BTC-000_AXC", 9e8, 10e8)
	res, err = testClient.DeliverTxSync(msg, testApp.Codec)

	msg = order.NewNewOrderMsg(addr2, GetOrderId(addr2, 15, ctx), 1, "BTC-000_AXC", 9e8, 10e8)
	res, err = testClient.DeliverTxSync(msg, testApp.Codec)
	...
	assert.Equal(int64(98613.3745e8), GetAvail(ctx, addr2, "AXC"))
	assert.Equal(int64(0), GetLocked(ctx, addr2, "BTC-000"))
	assert.Equal(int64(135e8), GetLocked(ctx, addr2, "AXC"))
	assert.Equal(int64(99815e8), GetAvail(ctx, addr3, "BTC-000"))
	assert.Equal(int64(101185.4070e8), GetAvail(ctx, addr3, "AXC"))
	assert.Equal(int64(10e8), GetLocked(ctx, addr3, "BTC-000"))
	assert.Equal(int64(0), GetLocked(ctx, addr3, ""))
	...
	*/
	...
	msg = order.NewNewOrderMsg(addr2, GetOrderId(addr2, 16, ctx), 1, "BTC-000_AXC", 20e8, 50e8)
	res, err = testClient.DeliverTxSync(msg, testApp.Codec)

	msg = order.NewNewOrderMsg(addr3, GetOrderId(addr3, 5, ctx), 2, "BTC-000_AXC", 10e8, 10e8)
	res, err = testClient.DeliverTxSync(msg, testApp.Codec)

	msg = order.NewNewOrderMsg(addr3, GetOrderId(addr3, 6, ctx), 2, "BTC-000_AXC", 9e8, 10e8)
	res, err = testClient.DeliverTxSync(msg, testApp.Codec)

	msg = order.NewNewOrderMsg(addr3, GetOrderId(addr3, 7, ctx), 2, "BTC-000_AXC", 7e8, 10e8)
	res, err = testClient.DeliverTxSync(msg, testApp.Codec)
	...
	assert.Equal(int64(97973.1545e8), GetAvail(ctx, addr2, "AXC"))
	assert.Equal(int64(0), GetLocked(ctx, addr2, "BTC-000"))
	assert.Equal(int64(335e8), GetLocked(ctx, addr2, "AXC"))
	assert.Equal(int64(99785e8), GetAvail(ctx, addr3, "BTC-000"))
	assert.Equal(int64(101625.1870e8), GetAvail(ctx, addr3, "AXC"))
	...
	*/
	...
	msg = order.NewNewOrderMsg(addr3, GetOrderId(addr3, 8, ctx), 2, "BTC-000_AXC", 17e8, 10e8)
	res, err = testClient.DeliverTxSync(msg, testApp.Codec)

	msg = order.NewNewOrderMsg(addr2, GetOrderId(addr2, 17, ctx), 1, "BTC-000_AXC", 15e8, 30e8)
	res, err = testClient.DeliverTxSync(msg, testApp.Codec)

	msg = order.NewNewOrderMsg(addr3, GetOrderId(addr3, 9, ctx), 2, "BTC-000_AXC", 15e8, 10e8)
	res, err = testClient.DeliverTxSync(msg, testApp.Codec)

	msg = order.NewNewOrderMsg(addr3, GetOrderId(addr3, 10, ctx), 2, "BTC-000_AXC", 9e8, 30e8)
	res, err = testClient.DeliverTxSync(msg, testApp.Codec)
	...
	assert.Equal(int64(97572.8545e8), GetAvail(ctx, addr2, "AXC"))
	assert.Equal(int64(0), GetLocked(ctx, addr2, "BTC-000"))
	assert.Equal(int64(135e8), GetLocked(ctx, addr2, "AXC"))
	assert.Equal(int64(99735e8), GetAvail(ctx, addr3, "BTC-000"))
	assert.Equal(int64(102224.8870e8), GetAvail(ctx, addr3, "AXC"))
	assert.Equal(int64(10e8), GetLocked(ctx, addr3, "BTC-000"))
	assert.Equal(int64(0), GetLocked(ctx, addr3, "AXC"))
	...
	*/
	...
	msg = order.NewNewOrderMsg(addr2, GetOrderId(addr2, 18, ctx), 1, "BTC-000_AXC", 17e8, 10e8)
	res, err = testClient.DeliverTxSync(msg, testApp.Codec)

	msg = order.NewNewOrderMsg(addr3, GetOrderId(addr3, 11, ctx), 2, "BTC-000_AXC", 16e8, 10e8)
	res, err = testClient.DeliverTxSync(msg, testApp.Codec)

	msg = order.NewNewOrderMsg(addr2, GetOrderId(addr2, 19, ctx), 1, "BTC-000_AXC", 15e8, 10e8)
	res, err = testClient.DeliverTxSync(msg, testApp.Codec)

	msg = order.NewNewOrderMsg(addr3, GetOrderId(addr3, 12, ctx), 2, "BTC-000_AXC", 15e8, 10e8)
	res, err = testClient.DeliverTxSync(msg, testApp.Codec)

	msg = order.NewNewOrderMsg(addr2, GetOrderId(addr2, 20, ctx), 1, "BTC-000_AXC", 11e8, 10e8)
	res, err = testClient.DeliverTxSync(msg, testApp.Codec)

	msg = order.NewNewOrderMsg(addr3, GetOrderId(addr3, 13, ctx), 2, "BTC-000_AXC", 9e8, 30e8)
	res, err = testClient.DeliverTxSync(msg, testApp.Codec)
	...
	assert.Equal(int64(97242.6895e8), GetAvail(ctx, addr2, "AXC"))
	assert.Equal(int64(0), GetLocked(ctx, addr2, "BTC-000"))
	assert.Equal(int64(135e8), GetLocked(ctx, addr2, "AXC"))
	assert.Equal(int64(99685e8), GetAvail(ctx, addr3, "BTC-000"))
	assert.Equal(int64(102554.7220e8), GetAvail(ctx, addr3, "AXC"))
	assert.Equal(int64(30e8), GetLocked(ctx, addr3, "BTC-000"))
	assert.Equal(int64(0), GetLocked(ctx, addr3, "AXC"))
	...
	*/
	...
	msg = order.NewNewOrderMsg(addr2, GetOrderId(addr2, 21, ctx), 1, "BTC-000_AXC", 17e8, 30e8)
	res, err = testClient.DeliverTxSync(msg, testApp.Codec)
	...
	assert.Equal(int64(96732.4345e8), GetAvail(ctx, addr2, "AXC"))
	assert.Equal(int64(0), GetLocked(ctx, addr2, "BTC-000"))
	assert.Equal(int64(135e8), GetLocked(ctx, addr2, "AXC"))
	assert.Equal(int64(99685e8), GetAvail(ctx, addr3, "BTC-000"))
	assert.Equal(int64(103064.4670e8), GetAvail(ctx, addr3, "AXC"))
	assert.Equal(int64(0), GetLocked(ctx, addr3, "BTC-000"))
	assert.Equal(int64(0), GetLocked(ctx, addr3, "AXC"))
	...
	*/
	...
	msg = order.NewNewOrderMsg(addr2, GetOrderId(addr2, 22, ctx), 1, "BTC-000_AXC", 17e8, 10e8)
	res, err = testClient.DeliverTxSync(msg, testApp.Codec)

	msg = order.NewNewOrderMsg(addr2, GetOrderId(addr2, 23, ctx), 1, "BTC-000_AXC", 12e8, 10e8)
	res, err = testClient.DeliverTxSync(msg, testApp.Codec)

	msg = order.NewNewOrderMsg(addr3, GetOrderId(addr3, 14, ctx), 2, "BTC-000_AXC", 10e8, 30e8)
	res, err = testClient.DeliverTxSync(msg, testApp.Codec)
	...
	assert.Equal(int64(96492.3145e8), GetAvail(ctx, addr2, "AXC"))
	assert.Equal(int64(0), GetLocked(ctx, addr2, "BTC-000"))
	assert.Equal(int64(135e8), GetLocked(ctx, addr2, "AXC"))
	assert.Equal(int64(99655e8), GetAvail(ctx, addr3, "BTC-000"))
	assert.Equal(int64(103304.3470e8), GetAvail(ctx, addr3, "AXC"))
	assert.Equal(int64(10e8), GetLocked(ctx, addr3, "BTC-000"))
	assert.Equal(int64(0), GetLocked(ctx, addr3, "AXC"))
}
...
func Test_Match_Allocation_new(t *testing.T) {
	...
	*/
	...
	assert.Equal(int64(100130e8), GetAvail(ctx, addr2, "BTC-000"))
	assert.Equal(int64(99199.6000e8), GetAvail(ctx, addr2, "AXC"))
	assert.Equal(int64(0), GetLocked(ctx, addr2, "BTC-000"))
	assert.Equal(int64(0), GetLocked(ctx, addr2, "AXC"))
	assert.Equal(int64(99850e8), GetAvail(ctx, addr3, "BTC-000"))
	assert.Equal(int64(100739.63e8), GetAvail(ctx, addr3, "AXC"))
	...
	assert.Equal(int64(0), GetLocked(ctx, addr3, "AXC"))
	assert.Equal(int64(99990e8), GetAvail(ctx, addr1, "BTC-000"))
	assert.Equal(int64(100059.9698e8), GetAvail(ctx, addr1, "AXC"))
	assert.Equal(int64(0), GetLocked(ctx, addr1, "BTC-000"))
	assert.Equal(int64(0), GetLocked(ctx, addr1, "AXC"))
	...
	*/
	...
	assert.Equal(int64(100150e8), GetAvail(ctx, addr2, "BTC-000"))
	assert.Equal(int64(99044.5225e8), GetAvail(ctx, addr2, "AXC"))
	assert.Equal(int64(0), GetLocked(ctx, addr2, "BTC-000"))
	assert.Equal(int64(0), GetLocked(ctx, addr2, "AXC"))
	assert.Equal(int64(99825e8), GetAvail(ctx, addr3, "BTC-000"))
	assert.Equal(int64(100894.5525e8), GetAvail(ctx, addr3, "AXC"))
	assert.Equal(int64(35e8), GetLocked(ctx, addr3, "BTC-000"))
	assert.Equal(int64(0), GetLocked(ctx, addr3, "AXC"))
	...
	*/
	...
	assert.Equal(int64(100170e8), GetAvail(ctx, addr2, "BTC-000"))
	assert.Equal(int64(98884.4425e8), GetAvail(ctx, addr2, "AXC"))
	assert.Equal(int64(0), GetLocked(ctx, addr2, "BTC-000"))
	assert.Equal(int64(0), GetLocked(ctx, addr2, "AXC"))
	assert.Equal(int64(99815e8), GetAvail(ctx, addr3, "BTC-000"))
	assert.Equal(int64(101054.4725e8), GetAvail(ctx, addr3, "AXC"))
	assert.Equal(int64(25e8), GetLocked(ctx, addr3, "BTC-000"))
	assert.Equal(int64(0), GetLocked(ctx, addr3, "AXC"))
	...
	*/
	...
	assert.Equal(int64(100185e8), GetAvail(ctx, addr2, "BTC-000"))
	assert.Equal(int64(98629.3825e8), GetAvail(ctx, addr2, "AXC"))
	assert.Equal(int64(0), GetLocked(ctx, addr2, "BTC-000"))
	assert.Equal(int64(135e8), GetLocked(ctx, addr2, "AXC"))
	assert.Equal(int64(99815e8), GetAvail(ctx, addr3, "BTC-000"))
	assert.Equal(int64(101174.4125e8), GetAvail(ctx, addr3, "AXC"))
	assert.Equal(int64(10e8), GetLocked(ctx, addr3, "BTC-000"))
	assert.Equal(int64(0), GetLocked(ctx, addr3, "AXC"))
	...
	*/
	...
	assert.Equal(int64(100225e8), GetAvail(ctx, addr2, "BTC-000"))
	assert.Equal(int64(97989.1625e8), GetAvail(ctx, addr2, "AXC"))
	assert.Equal(int64(0), GetLocked(ctx, addr2, "BTC-000"))
	assert.Equal(int64(335e8), GetLocked(ctx, addr2, "AXC"))
	assert.Equal(int64(99785e8), GetAvail(ctx, addr3, "BTC-000"))
	assert.Equal(int64(101614.1925e8), GetAvail(ctx, addr3, "AXC"))
	assert.Equal(int64(0), GetLocked(ctx, addr3, "BTC-000"))
	assert.Equal(int64(0), GetLocked(ctx, addr3, "AXC"))
	...
	*/
	...
	assert.Equal(int64(100265e8), GetAvail(ctx, addr2, "BTC-000"))
	assert.Equal(int64(97538.8375e8), GetAvail(ctx, addr2, "AXC"))
	assert.Equal(int64(0), GetLocked(ctx, addr2, "BTC-000"))
	assert.Equal(int64(135e8), GetLocked(ctx, addr2, "AXC"))
	assert.Equal(int64(99735e8), GetAvail(ctx, addr3, "BTC-000"))
	assert.Equal(int64(102263.8675e8), GetAvail(ctx, addr3, "AXC"))
	assert.Equal(int64(10e8), GetLocked(ctx, addr3, "BTC-000"))
	assert.Equal(int64(0), GetLocked(ctx, addr3, "AXC"))
	...
	*/
	...
	assert.Equal(int64(100295e8), GetAvail(ctx, addr2, "BTC-000"))
	assert.Equal(int64(97208.6725e8), GetAvail(ctx, addr2, "AXC"))
	assert.Equal(int64(0), GetLocked(ctx, addr2, "BTC-000"))
	assert.Equal(int64(135e8), GetLocked(ctx, addr2, "AXC"))
	assert.Equal(int64(99685e8), GetAvail(ctx, addr3, "BTC-000"))
	assert.Equal(int64(102593.7025e8), GetAvail(ctx, addr3, "AXC"))
	assert.Equal(int64(30e8), GetLocked(ctx, addr3, "BTC-000"))
	assert.Equal(int64(0), GetLocked(ctx, addr3, "AXC"))
	...
	*/
	...
	assert.Equal(int64(100325e8), GetAvail(ctx, addr2, "BTC-000"))
	assert.Equal(int64(96728.4325e8), GetAvail(ctx, addr2, "AXC"))
	assert.Equal(int64(0), GetLocked(ctx, addr2, "BTC-000"))
	assert.Equal(int64(135e8), GetLocked(ctx, addr2, "AXC"))
	assert.Equal(int64(99685e8), GetAvail(ctx, addr3, "BTC-000"))
	assert.Equal(int64(103073.4625e8), GetAvail(ctx, addr3, "AXC"))
	assert.Equal(int64(0), GetLocked(ctx, addr3, "BTC-000"))
	assert.Equal(int64(0), GetLocked(ctx, addr3, "AXC"))
	...
	*/
	...
	assert.Equal(int64(100345e8), GetAvail(ctx, addr2, "BTC-000"))
	assert.Equal(int64(96488.3125e8), GetAvail(ctx, addr2, "AXC"))
	assert.Equal(int64(0), GetLocked(ctx, addr2, "BTC-000"))
	assert.Equal(int64(135e8), GetLocked(ctx, addr2, "AXC"))
	assert.Equal(int64(99655e8), GetAvail(ctx, addr3, "BTC-000"))
	assert.Equal(int64(103313.3425e8), GetAvail(ctx, addr3, "AXC"))
	assert.Equal(int64(10e8), GetLocked(ctx, addr3, "BTC-000"))
	assert.Equal(int64(0), GetLocked(ctx, addr3, "AXC"))
}
