...
import (
	...
	o "github.com/aximchain/flash-node/plugins/dex/order"
...
func Test_handleNewOrder_CheckTx(t *testing.T) {
	...
	testApp.DexKeeper.PairMapper.AddTradingPair(ctx, types.NewTradingPair("BTC-000", "AXC", 1e8))
	...
	msg := o.NewNewOrderMsg(add, genOrderID(add, 0, ctx, am), 1, "BTC-000_AXC", 355e8, 100e8)
	...
	assert.Equal(int64(500e8), GetAvail(ctx, add, "AXC"))
	assert.Equal(int64(0), GetLocked(ctx, add, "AXC"))
	assert.Equal(int64(200e8), GetAvail(ctx, add, "BTC-000"))
	assert.Equal(int64(0), GetLocked(ctx, add, "BTC-000"))

	msg = o.NewNewOrderMsg(add, genOrderID(add, 0, ctx, am), 1, "BTC-000_AXC", 355e8, 1e8)
	res, e = testClient.CheckTxSync(msg, testApp.Codec)
	assert.Equal(uint32(0), res.Code)
	assert.Nil(e)
	assert.Equal(int64(145e8), GetAvail(ctx, add, "AXC"))
	assert.Equal(int64(355e8), GetLocked(ctx, add, "AXC"))
	assert.Equal(int64(200e8), GetAvail(ctx, add, "BTC-000"))
	assert.Equal(int64(0), GetLocked(ctx, add, "BTC-000"))

	// using acc2

	msg = o.NewNewOrderMsg(add2, genOrderID(add2, 0, ctx, am), 2, "BTC-000_AXC", 355e8, 250e8)
	res, e = testClient.CheckTxSync(msg, testApp.Codec)
	assert.NotEqual(uint32(0), res.Code)
	assert.Nil(e)
	assert.Regexp(".*do not have enough token to lock.*", res.GetLog())
	assert.Equal(int64(500e8), GetAvail(ctx, add2, "AXC"))
	assert.Equal(int64(0), GetLocked(ctx, add2, "AXC"))
	assert.Equal(int64(200e8), GetAvail(ctx, add2, "BTC-000"))
	assert.Equal(int64(0), GetLocked(ctx, add2, "BTC-000"))

	msg = o.NewNewOrderMsg(add2, genOrderID(add2, 0, ctx, am), 2, "BTC-000_AXC", 355e8, 200e8)
	res, e = testClient.CheckTxSync(msg, testApp.Codec)
	assert.Equal(uint32(0), res.Code)
	assert.Nil(e)
	assert.Equal(int64(500e8), GetAvail(ctx, add2, "AXC"))
...
func Test_handleNewOrder_DeliverTx(t *testing.T) {
	...
	buys, sells, pendingMatch = getOrderBook("ETH-001_AXC")
...
func Test_Match(t *testing.T) {
	...
	InitAccounts(ctx, testApp)
	testApp.DexKeeper.ClearOrderBook("BTC-000_AXC")
	ethPair := types.NewTradingPair("ETH-000", "AXC", 97e8)
	testApp.DexKeeper.PairMapper.AddTradingPair(ctx, ethPair)
	testApp.DexKeeper.AddEngine(ethPair)
	testApp.DexKeeper.GetEngines()["ETH-000_AXC"].LastMatchHeight = -1
	btcPair := types.NewTradingPair("BTC-000", "AXC", 96e8)
	...
	*/
	...
	t.Log(GetAvail(ctx, add, "AXC"))
	msg := o.NewNewOrderMsg(add, genOrderID(add, 0, ctx, am), 1, "BTC-000_AXC", 102e8, 300e8)
	res, e := testClient.DeliverTxSync(msg, testApp.Codec)
	t.Log(GetAvail(ctx, add, "BTC-000"))
	t.Log(GetAvail(ctx, add, "AXC"))
	msg = o.NewNewOrderMsg(add, genOrderID(add, 1, ctx, am), 1, "BTC-000_AXC", 100e8, 100e8)
	res, e = testClient.DeliverTxSync(msg, testApp.Codec)
	t.Log(GetAvail(ctx, add, "BTC-000"))
	t.Log(GetAvail(ctx, add, "AXC"))

	msg = o.NewNewOrderMsg(add2, genOrderID(add2, 0, ctx, am), 2, "BTC-000_AXC", 96e8, 1000e8)
	res, e = testClient.DeliverTxSync(msg, testApp.Codec)
	msg = o.NewNewOrderMsg(add2, genOrderID(add2, 1, ctx, am), 2, "BTC-000_AXC", 97e8, 250e8)
	res, e = testClient.DeliverTxSync(msg, testApp.Codec)
	msg = o.NewNewOrderMsg(add2, genOrderID(add2, 2, ctx, am), 2, "BTC-000_AXC", 98e8, 250e8)
	res, e = testClient.DeliverTxSync(msg, testApp.Codec)
	msg = o.NewNewOrderMsg(add, genOrderID(add, 2, ctx, am), 1, "BTC-000_AXC", 99e8, 200e8)
	res, e = testClient.DeliverTxSync(msg, testApp.Codec)
	t.Logf("res is %v and error is %v", res, e)
	msg = o.NewNewOrderMsg(add, genOrderID(add, 3, ctx, am), 1, "BTC-000_AXC", 98e8, 300e8)
	res, e = testClient.DeliverTxSync(msg, testApp.Codec)
	buys, sells, pendingMatch := getOrderBook("BTC-000_AXC")
	...
	// total execution is 900e8 BTC-000 @ price 96e8, notional is 86400e8, fee is 43.2e8
	assert.Equal(sdk.Coins{sdk.NewCoin("AXC", 86.4e8)}, fees.Pool.BlockFees().Tokens)
	assert.Equal(int64(100900e8), GetAvail(ctx, add, "BTC-000"))
	assert.Equal(int64(13556.8e8), GetAvail(ctx, add, "AXC"))
	assert.Equal(int64(0), GetLocked(ctx, add, "BTC-000"))
	assert.Equal(int64(0), GetLocked(ctx, add, "AXC"))
	assert.Equal(int64(98500e8), GetAvail(ctx, add2, "BTC-000"))
	assert.Equal(int64(186356.8e8), GetAvail(ctx, add2, "AXC"))
	assert.Equal(int64(600e8), GetLocked(ctx, add2, "BTC-000"))
	assert.Equal(int64(0), GetLocked(ctx, add2, "AXC"))

	// test ETH-000_AXC pair
	/*	--------------------------------------------------------------
		SUM    SELL    PRICE    BUY    SUM    EXECUTION    IMBALANCE
		110            102      30     30     30           -80
		110            101      10     40     40           -70
	...
	*/
	...
	msg = o.NewNewOrderMsg(add2, genOrderID(add2, 3, ctx, am), 1, "ETH-000_AXC", 102e8, 30e8)
	res, e = testClient.DeliverTxSync(msg, testApp.Codec)
	msg = o.NewNewOrderMsg(add2, genOrderID(add2, 4, ctx, am), 1, "ETH-000_AXC", 101e8, 10e8)
	res, e = testClient.DeliverTxSync(msg, testApp.Codec)
	msg = o.NewNewOrderMsg(add3, genOrderID(add3, 0, ctx, am), 2, "ETH-000_AXC", 95e8, 50e8)
	res, e = testClient.DeliverTxSync(msg, testApp.Codec)
	msg = o.NewNewOrderMsg(add3, genOrderID(add3, 1, ctx, am), 2, "ETH-000_AXC", 98e8, 10e8)
	res, e = testClient.DeliverTxSync(msg, testApp.Codec)
	msg = o.NewNewOrderMsg(add3, genOrderID(add3, 2, ctx, am), 2, "ETH-000_AXC", 97e8, 50e8)
	res, e = testClient.DeliverTxSync(msg, testApp.Codec)
	msg = o.NewNewOrderMsg(add2, genOrderID(add2, 5, ctx, am), 1, "ETH-000_AXC", 96e8, 15e8)
	res, e = testClient.DeliverTxSync(msg, testApp.Codec)
	msg = o.NewNewOrderMsg(add2, genOrderID(add2, 6, ctx, am), 1, "ETH-000_AXC", 99e8, 50e8)
	res, e = testClient.DeliverTxSync(msg, testApp.Codec)
	t.Logf("res is %v and error is %v", res, e)

	buys, sells, _ = getOrderBook("BTC-000_AXC")
	assert.Equal(0, len(buys))
	assert.Equal(3, len(sells))
	buys, sells, _ = getOrderBook("ETH-000_AXC")
	assert.Equal(4, len(buys))
	assert.Equal(3, len(sells))
	...
	assert.Equal(0, len(buys))
	assert.Equal(3, len(sells))
	trades, lastPx = testApp.DexKeeper.GetLastTradesForPair("ETH-000_AXC")
	assert.Equal(int64(97e8), lastPx)
	assert.Equal(4, len(trades))
	// total execution is 90e8 ETH @ price 97e8, notional is 8730e8
	// fee for this round is 8.73e8 , totalFee is 95.13e8
	assert.Equal(sdk.Coins{sdk.NewCoin("AXC", 95.13e8)}, fees.Pool.BlockFees().Tokens)
	fees.Pool.Clear()
	assert.Equal(int64(100900e8), GetAvail(ctx, add, "BTC-000"))
	assert.Equal(int64(13556.8e8), GetAvail(ctx, add, "AXC"))
	assert.Equal(int64(0), GetLocked(ctx, add, "BTC-000"))
	...
	// fee is 4.365e8
	assert.Equal(int64(176182.435e8), GetAvail(ctx, add2, "AXC"))
	assert.Equal(int64(1440e8), GetLocked(ctx, add2, "AXC"))
	...
	assert.Equal(int64(108725.635e8), GetAvail(ctx, add3, "AXC"))
	assert.Equal(int64(0), GetLocked(ctx, add3, "AXC"))
...
func Test_handleCancelOrder_CheckTx(t *testing.T) {
	...
	msg := o.NewCancelOrderMsg(add, "BTC-000_AXC", "DOESNOTEXIST-0")
	res, e := testClient.DeliverTxSync(msg, testApp.Codec)
	assert.Regexp(".*Failed to find order \\[DOESNOTEXIST-0\\].*", res.GetLog())
	assert.Nil(e)
	newMsg := o.NewNewOrderMsg(add, oid, 1, "BTC-000_AXC", 355e8, 1e8)
	...
	assert.Equal(int64(200e8), GetAvail(ctx, add, "BTC-000"))
	assert.Equal(int64(0), GetLocked(ctx, add, "BTC-000"))
	msg = o.NewCancelOrderMsg(add2, "BTC-000_AXC", oid)
	res, e = testClient.DeliverTxSync(msg, testApp.Codec)
	assert.Regexp(".*does not belong to transaction sender.*", res.GetLog())
	msg = o.NewCancelOrderMsg(add, "BTC-000_AXC", oid)
	res, e = testClient.DeliverTxSync(msg, testApp.Codec)
	assert.Equal(uint32(0), res.Code)
	assert.Nil(e)
	assert.Equal(int64(500e8-2e4), GetAvail(ctx, add, "AXC"))
	assert.Equal(int64(0), GetLocked(ctx, add, "AXC"))
	assert.Equal(int64(200e8), GetAvail(ctx, add, "BTC-000"))
	assert.Equal(int64(0), GetLocked(ctx, add, "BTC-000"))
}
