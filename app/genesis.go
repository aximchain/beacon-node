...
func AximchainAppInit() server.AppInit {
	return server.AppInit{
		AppGenState: AximchainAppGenState,
	}
}

// AppGenState sets up the app_state and appends the cool app state
func AximchainAppGenState(cdc *wire.Codec, appGenTxs []json.RawMessage) (appState json.RawMessage, err error) {
	if len(appGenTxs) == 0 {
