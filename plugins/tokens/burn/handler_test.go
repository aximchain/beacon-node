...
func TestHandleBurnMini(t *testing.T) {
	...
	token, err = tokenMapper.GetToken(ctx, "NNB-000M")
	require.NoError(t, err)
	expectedToken = types.NewMiniToken("New AXC", "NNB", "NNB-000M", 2, 1e8-1, acc.GetAddress(), false, "http://www.xyz.com/nnb.json")
	require.Equal(t, expectedToken, token)
	...
	token, err = tokenMapper.GetToken(ctx, "NNB-000M")
	require.NoError(t, err)
	expectedToken = types.NewMiniToken("New AXC", "NNB", "NNB-000M", 2, 0, acc.GetAddress(), false, "http://www.xyz.com/nnb.json")
	require.Equal(t, expectedToken, token)
...
func TestHandleBurn(t *testing.T) {
	...
	token, err = tokenMapper.GetToken(ctx, "NNB-000")
	require.NoError(t, err)
	expectedToken, err := types.NewToken("New AXC", "NNB-000", 1e8-1, acc.GetAddress(), false)
	require.Equal(t, expectedToken, token)
