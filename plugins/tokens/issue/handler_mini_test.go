...
func TestHandleIssueMiniToken(t *testing.T) {
	...
	msg := NewIssueTinyMsg(acc.GetAddress(), "New AXC", "NNB", 10000e8+100, false, "http://www.xyz.com/nnb.json")
	...
	ctx = ctx.WithValue(baseapp.TxHashKey, "000")
	msg = NewIssueTinyMsg(acc.GetAddress(), "New AXC", "NNB", 10000e8, false, "http://www.xyz.com/nnb.json")
	sdkResult = handler(ctx, msg)
	require.Equal(t, true, sdkResult.Code.IsOK())

	token, err := tokenMapper.GetToken(ctx, "NNB-000M")
	require.NoError(t, err)
	expectedToken := types.NewMiniToken("New AXC", "NNB", "NNB-000M", 1, 10000e8, acc.GetAddress(), false, "http://www.xyz.com/nnb.json")
...
func TestHandleMintMiniToken(t *testing.T) {
	...
	issueMsg := NewIssueTinyMsg(acc.GetAddress(), "New AXC", "NNB", 9000e8, true, "http://www.xyz.com/nnb.json")
	ctx = ctx.WithValue(baseapp.TxHashKey, "000")
	...
	token, err := tokenMapper.GetToken(ctx, "NNB-000M")
	require.NoError(t, err)
	expectedToken := types.NewMiniToken("New AXC", "NNB", "NNB-000M", 1, 9000e8, acc.GetAddress(), true, "http://www.xyz.com/nnb.json")
	require.Equal(t, expectedToken, token)
	...
	require.Equal(t, true, sdkResult.Code.IsOK())
	token, err = tokenMapper.GetToken(ctx, "NNB-000M")
	expectedToken = types.NewMiniToken("New AXC", "NNB", "NNB-000M", 1, 10000e8, acc.GetAddress(), true, "http://www.xyz.com/nnb.json")
	require.Equal(t, expectedToken, token)
	...
	issueMsg = NewIssueTinyMsg(acc.GetAddress(), "New AXC2", "NNB2", 9000e8, false, "http://www.xyz.com/nnb.json")
	ctx = ctx.WithValue(baseapp.TxHashKey, "000")
	sdkResult = handler(ctx, issueMsg)
	require.Equal(t, true, sdkResult.Code.IsOK())
