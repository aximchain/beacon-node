...
func TestHandleCreateAndClaimSwap(t *testing.T) {
	...
	amount := sdk.Coins{sdk.Coin{"AXC", 10000}}
	expectedIncome := "10000:AXC"
...
func TestHandleCreateAndClaimSwapForSingleChain(t *testing.T) {
	...
	amountAXC := sdk.Coins{sdk.Coin{"AXC", 10000}}
	expectedIncome := "100000000:ABC"
	heightSpan := int64(1000)

	var msg sdk.Msg
	msg = NewHTLTMsg(acc1.GetAddress(), acc2.GetAddress(), "", "", randomNumberHash, timestamp, amountAXC, expectedIncome, heightSpan, false)
	...
	AtomicSwapCoinsAcc := accKeeper.GetAccount(ctx, AtomicSwapCoinsAccAddr)
	require.Equal(t, amountAXC.Plus(amountABC).Sort(), AtomicSwapCoinsAcc.GetCoins())
	...
	require.Equal(t, amountAXC[0].Amount, acc1OrignalCoins.AmountOf("AXC")-acc1Acc.GetCoins().AmountOf("AXC"))

	acc2Acc := accKeeper.GetAccount(ctx, acc2.GetAddress())
	require.Equal(t, amountAXC[0].Amount, acc2Acc.GetCoins().AmountOf("AXC")-acc2OrignalCoins.AmountOf("AXC"))
